...
class User(db.Model):
    ...
    point_balance = db.Column(db.Float(64), default=0)
    credit_balance = db.Column(db.Float(64), default=0)
    donated_points = db.Column(db.Float(64), default=0)
    lifetime_points_field = db.Column('lifetime_points', db.Float(64), default=0)
    ...
    def get_points(self):
        # return int(self.point_balance)
        points = self.point_balance
        if points is None:
            return "-1"
        res = str(points)
        return res
    ...
    def is_below_tier(self, tier):
        ...
        total_tier = ("Carbon", "Bronze", "Silver", "Gold", "Platinum")

        if current_tier in total_tier and tier in total_tier:
            current_index = total_tier.index(current_tier)
            tier_index = total_tier.index(tier)

            if current_index<tier_index:
                return True
